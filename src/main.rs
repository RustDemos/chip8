...
fn main() {
    ...
    let mut window = Window::new("Chip8 emulator", WIDTH, HEIGHT, WindowOptions::default())
        .unwrap_or_else(|e| {
    ...
    while window.is_open() && !window.is_key_down(Key::Escape) {
        ...
        if Instant::now() - last_display_time > Duration::from_millis(1 / 60) {
            let chip8_buffer = chip8.get_display_buffer();

            for y in 0..HEIGHT {
                let y_coord = y / (HEIGHT / 32);
                let offset = y * WIDTH;
                for x in 0..WIDTH {
                    let index = Display::get_index_from_coords(x / (WIDTH / 64), y_coord);
